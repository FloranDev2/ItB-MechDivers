...
-- CUSTOM FUNCTION WRITTEN BY TRUELCH
-- p: pawn space or id
-- mode: mode id (string)
-- isActive: true: will show the mode (default) / false: will hide the mode's icon.
function aFM_WeaponTemplate:FM_SetActive(p, mode, isActive)
	local m = GetCurrentMission()
	local weaponIdx = this:GetSkillIdx(p, self)
	local pId = Board:GetPawn(p):GetId()
